...
void printSchedule()
{
    ...
    {
        /* Same task + same job → extend interval */
...
for(int i=1;i<schedSize;i++){
if(sched[i].taskId!=sched[i-1].taskId){
cs++;
...
/* H */
int total=sched[schedSize-1].end,idle=0;
for(int i=0;i<schedSize;i++)
if(sched[i].taskId==-1)
idle+=sched[i].end-sched[i].start;
fprintf(fp,"CPU Util:%.3f\n",(double)(total-idle)/total);

/* E,F,I,J */
int minRT[50],maxRT[50],minWT[50],maxWT[50],minLat[50],maxLat[50],minLate[50],maxLate[50];
double sumRT[50]={0},sumWT[50]={0},sumLat[50]={0},sumLate[50]={0};
int cnt[50]={0};

for(int i=0;i<50;i++){
minRT[i]=minWT[i]=minLat[i]=1e9;
maxRT[i]=maxWT[i]=maxLat[i]=0;
minLate[i]=1e9; maxLate[i]=-1e9;
}
...
sumRT[t]+=rt; sumWT[t]+=wt;
sumLat[t]+=lat; sumLate[t]+=late;
cnt[t]++;
}
